  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-2">온라인 강의</h1>
          <p className="text-xl text-gray-600 dark:text-gray-400">대학별 논술 전략과 실전 기술을 배워보세요</p>
        </div>

        {/* Filters */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-8 transition-colors">
          <div className="flex items-center mb-4">
            <FiFilter className="w-5 h-5 text-gray-600 dark:text-gray-400 mr-2" />
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">필터</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">난이도</label>
              <select
                value={filter.level || ''}
                onChange={(e) =>
                  setFilter((prev) => ({
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCourses.map((course) => (
            <Link
              ...
              className="bg-white dark:bg-gray-800 rounded-lg shadow hover:shadow-xl transition-all overflow-hidden group"
            >
              ...
              <div className="p-6">
                ...
                <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 mb-3">
                  <span className="font-medium text-gray-900 dark:text-gray-200">{course.instructor}</span>
                </div>

                <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400 mb-4">
                  <div className="flex items-center">
                    <FiStar className="w-4 h-4 text-yellow-500 dark:text-yellow-400 mr-1" />
                    <span className="font-medium text-gray-900 dark:text-gray-200">{course.rating}</span>
        ...
        {filteredCourses.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400">검색 조건에 맞는 강의가 없습니다.</p>
          </div>
        )}
      </div>
    </div>
